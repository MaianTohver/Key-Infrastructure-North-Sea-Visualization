...
if "headerkeys" not in st.session_state or "Summary2040" not in st.session_state:
    try:
        st.session_state["Summary2040"] = pd.read_pickle("data/summary_2040.pkl")
        st.session_state["Summary2030"] = st.session_state["Summary2040"].copy()
        st.session_state["headerkeys"] = pd.read_csv("data/header_keys.csv")
...
case_keys = {
    "2030": {"costs": "min costs"},
    "2040": {"costs": "min costs"}
}
...
with st.expander("Explanations"):
    ...
    st.markdown(
        "**Costs** optimizations minimize total system costs, composed of fixed and variable infrastructure expenses alongside negative emission asset implementations.")
    st.subheader("Scenarios")
    st.markdown(
        "In the **Baseline** scenario, no negative emission tech is allowed. In the **Onshore DAC** scenario, carbon capture facilities are bound to terrestrial grid points. In the **Offshore DAC** scenario, capture facilities are integrated out at sea with marine generation hubs.")
...
cols = [("global", "global", "Case"), ("global", "global", "Subcase"), ("global", "global", "climate_year")]
cols.extend([(variables_available.loc[key]["level0"], variables_available.loc[key]["level1"],
              variables_available.loc[key]["level2"]) for key in variables_selected])
...
if len(variables_selected) > 0:
    ...
    if not plot_data_other_cy.empty:
    ...
    if unstack_cols:
        ...
        if not plot_data_other_cy.empty:
            plot_data_other_cy_summed = plot_data_other_cy.groupby(["climate_year", "Case_Subcase"])[
